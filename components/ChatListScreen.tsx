...
import type { User, Chat, Message, CustomElement, PrivacySettings, PremiumScreenSettings } from '../types';
...
const getChatDisplayInfo = (chat: Chat, currentUserId: string): { name: string, photoURL: string | null, user?: User | null } => {
    ...
    return { 
        ...
        user: { ...otherUser, uid: otherUserId } as User
    };
}
...
const NameWithBadges: React.FC<{ user?: Partial<User> | null, name: string }> = ({ user, name }) => (
    <div className="flex items-center gap-1.5">
        <span className="font-semibold text-black dark:text-white truncate">{name}</span>
...
const ChatListItem: React.FC<{
    chat: Chat;
    currentUser: User;
    handleItemClick: (chat: Chat) => void;
    handleLongPress: (e: React.MouseEvent, chatId: string) => void;
    isSelected: boolean;
}> = ({ chat, currentUser, handleItemClick, handleLongPress, isSelected }) => {
    ...
    useEffect(() => {
        ...
        const listener = onValue(userRef, (snapshot) => {
            if (snapshot.exists()) {
                setOtherUser({ uid: snapshot.key, ...snapshot.val() });
            }
            setLoading(false);
        });
        userListenerUnsubscribe = () => off(userRef, 'value', listener);
    ...
    let photoURL = displayInfo.photoURL;

    if (chat.type === ChatType.Private) {
        // For a private chat, we consider them a contact.
        if (!canSeeInfo('profilePhoto', otherUser, true)) {
            photoURL = null;
        }
    }
    ...
    return (
        ...
        >
            <div className="relative mr-4">
                <Avatar photoURL={photoURL} name={displayInfo.name} sizeClass="w-14 h-14" />
                {isSelected && (
            ...
            <div className="flex-1 min-w-0">
                <NameWithBadges user={otherUser} name={displayInfo.name} />
...
const ChatListScreen: React.FC<ChatListScreenProps> = ({ currentUser, onSelectChat, onNavigate, appSettings, onTriggerUndo, theme, setTheme, startInSearchMode }) => {
  ...
  const [searchActive, setSearchActive] = useState(startInSearchMode || false);
        ...
        Promise.all(chatPromises).then(chatSnaps => {
            const chatsData = chatSnaps
                .map(snap => {
                    ...
                    chat.isMuted = typeof userChatInfo === 'object' && userChatInfo.muted;
    ...
    const handleSearch = async () => {
      ...
      try {
          ...
          if (userSnapshot && userSnapshot.exists()) {
            publicResultsData.push(...Object.values(userSnapshot.val() as { [key: string]: User }).filter(u => u.uid !== currentUser.uid && u.isPublic !== false));
          }
  ...
  const renderSearchResultItem = (item: User | Chat) => {
      ...
      return (
          <div key={key} onClick={() => handleSelectSearchResult(item)} className="flex items-center px-4 py-3 hover:bg-gray-100 dark:hover:bg-[#2f2f2f] cursor-pointer">
              ...
              <div className="flex-1 min-w-0">
                  {isUser(item) ? <NameWithBadges user={item} name={name} /> : <p className="font-semibold text-black dark:text-white truncate">{name}</p>}
  ...
  const areAllSelectedMuted = useMemo(() => {
    if (selectedChats.size === 0) return false;
    return Array.from(selectedChats).every(chatId => chats.find(c => c.id === chatId)?.isMuted);
  }, [selectedChats, chats]);
          ...
          for (const chatId of selectedArr) {
              ...
              switch(action) {
                  case 'pin':
                    ...
                    break;
                  case 'mute':
                      updates[`/user-chats/${currentUser.uid}/${chatId}/isMuted`] = !areAllSelectedMuted;
                      break;
                  case 'archive':
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-[#1e1e1e] text-black dark:text-white relative">
      <Drawer isOpen={isDrawerOpen} onClose={() => setIsDrawerOpen(false)} currentUser={currentUser} onNavigate={(view) => { onNavigate(view); setIsDrawerOpen(false); }} theme={theme} setTheme={setTheme} appSettings={appSettings} />
        ...
        {selectionMode ? (
            <div className="flex items-center justify-between space-x-2 relative">
                ...
                <div className="flex items-center space-x-1">
                    ...
                    <button onClick={() => handleSelectionAction('mute')} className="p-2 rounded-full hover:bg-black/10 dark:hover:bg-white/10">{areAllSelectedMuted ? <HeaderUnmuteIcon /> : <HeaderMuteIcon />}</button>
